...
class MobileNetv2(nn.Module):
    def __init__(self,width_mult=1,num_classes = 100):
        ...
        interverted_residual_setting = [
            ...
            [6, 160, 3, 2],
            [6, 320, 1, 1]]
        input_chanel = int(input_chanel * width_mult)
        head_layer = Head(3,input_chanel)
        self.layers = [head_layer]
        for t,c,n,s in interverted_residual_setting:
            stride = s
            output_chanel = int(c * width_mult)
            for i in range(n):
                if i == 0:
                    self.layers.append(block(input_chanel,output_chanel,stride,t))
                else:
                    self.layers.append(block(input_chanel,output_chanel,1,t))
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("X:\MobileNet\mobilenetv2-trace.pt"))
